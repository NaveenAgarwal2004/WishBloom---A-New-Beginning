...
export const MessageSchema = z.object({
  ...
  greeting: z.string().max(100).optional().or(z.literal('')),
  content: z.string().min(10, 'Content must be at least 10 characters').max(5000),
  closing: z.string().max(100).optional().or(z.literal('')),
  signature: z.string().min(1, 'Signature is required').max(100),
  title: z.string().max(200).optional().or(z.literal('')),
  postscript: z.string().max(500).optional().or(z.literal('')),
  contributor: ContributorSchema,
...
export const CreateWishBloomSchema = z.object({
  ...
  creativeAgeDescription: z.string().max(100).optional().or(z.literal('')),
